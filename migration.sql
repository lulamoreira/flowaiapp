...
-- Include in realtime
-- Note: Supabase UI handles this usually, but we can ensure the table is in the publication
ALTER PUBLICATION supabase_realtime ADD TABLE public.placeholder_members;

-- 2. CORREÇÃO DO GATILHO DE NOTIFICAÇÃO
-- Ajuste da função notify_task_assigned
...
BEGIN
  ...
  -- c) Migrar tarefas (suprimindo notificações)
  -- Para suprimir, poderíamos usar um SET local para uma variável de sessão, mas aqui
  -- como a função é curta, basta garantir que o ID seja trocado.
  -- O gatilho notify_task_assigned verifica EXISTENCE em auth.users, 
  -- então ele funcionará automaticamente para o _real_user_id.
  ...
  -- e) Mover participações de board (board_members se existir)
  -- NOTA: O schema não listou board_members no read_query anterior.
  -- Se não existir, ignoramos silenciosamente esta parte.
  -- Mas o pedido exigiu, então tentaremos.
